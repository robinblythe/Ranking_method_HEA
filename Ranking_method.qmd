...
```{r}
# Low estimate
nmb_simulation_low <- do_nmb_sim(
  sample_size = sample_size,
  n_sims = 500,
  n_valid = 10000,
  sim_auc = auc,
  event_rate = p0,
  fx_nmb_training = fx_nmb_low,
  fx_nmb_evaluation = sampler_low
)
summary(nmb_simulation_low)
autoplot(nmb_simulation_low, what = "inb", inb_ref_col = "none") + theme_sim()
autoplot(nmb_simulation_low, what = "cutpoints") + theme_sim()

cutpoint_youden_low <- median(nmb_simulation_low$df_thresholds$youden)
cutpoint_nmb_low <- median(nmb_simulation_low$df_thresholds$value_optimising)
```

```{r}
# High estimate
nmb_simulation_high <- do_nmb_sim(
  sample_size = sample_size,
  n_sims = 500,
  n_valid = 10000,
  sim_auc = auc,
  event_rate = p0,
  fx_nmb_training = fx_nmb_high,
  fx_nmb_evaluation = sampler_high
)
summary(nmb_simulation_high)
autoplot(nmb_simulation_high, what = "inb", inb_ref_col = "none") + theme_sim()
autoplot(nmb_simulation_high, what = "cutpoints") + theme_sim()

cutpoint_youden_high <- median(nmb_simulation_high$df_thresholds$youden)
cutpoint_nmb_high <- median(nmb_simulation_high$df_thresholds$value_optimising)
```
...
```{r, echo = FALSE, message = FALSE}
results_low <- obtain_payoffs(df_sims, sampler_low())
results_high <- obtain_payoffs(df_sims, sampler_high())

# Switch to long format for the classrank
...
# Plot results
p_low <- results_low |> ggplot(aes(x = Patients_evaluated))
p_high <- results_high |> ggplot(aes(x = Patients_evaluated))
