...
remove(serp_roc, cutpoint_serp)
```

### Results

Now that we have data from a real example, we can estimate the results as per the simulation above. Assuming the SGH ED sees 50 patients at any given time. The analysis from the simulation can be repeated; in a situation where a doctor asks for a consult, we can simulate the effect of assessing patients ranked by predicted risk, vs assessing all patients above the threshold (as per the external validation study above).

```{r}
# In this scenario, deterioration leads to ICU admission
df_ed <- df_ed |>
  mutate(
...
Assuming 100 patients at any given time are being assessed in ED

```{r}
# Set up bootstrap resampler
n_samples = 100
n_eval = 0.2 * n_samples
max_iter = 10000
results_out <- list()

# 1. Obtain random set
# 2. Extract all positive cases under each threshold (0.7, 0.8, 0.9)
# 3. Summarise PPV and misclassification costs using ranking vs under a random sample
set.seed(888)
for (i in 1:max_iter) {
  ...
  # Take (n_eval) random draws vs 5 ranking-based draws
  df_7 = subset(df, t1 == 1) |> slice_sample(n = n_eval) |> 
    mutate(Method = "t1 unranked", TP = ifelse(class_t1 == "TP", 1, 0), iter = i)
  df_8 = subset(df, t2 == 1) |> slice_sample(n = n_eval) |> 
    mutate(Method = "t2 unranked", TP = ifelse(class_t1 == "TP", 1, 0), iter = i)
  df_9 = subset(df, t3 == 1) |> slice_sample(n = n_eval) |> 
    mutate(Method = "t3 unranked", TP = ifelse(class_t1 == "TP", 1, 0), iter = i)
  df_7_rank = subset(df, t1 == 1) |> slice_head(n = n_eval) |> 
    mutate(Method = "t1 ranked", TP = ifelse(class_t1 == "TP", 1, 0), iter = i)
  df_8_rank = subset(df, t2 == 1) |> slice_head(n = n_eval) |> 
    mutate(Method = "t2 ranked", TP = ifelse(class_t1 == "TP", 1, 0), iter = i)
  df_9_rank = subset(df, t3 == 1) |> slice_head(n = n_eval) |> 
    mutate(Method = "t3 ranked", TP = ifelse(class_t1 == "TP", 1, 0), iter = i)
  
  results_out[[i]] <- do.call(rbind, list(df_7, df_8, df_9, df_7_rank, df_8_rank, df_9_rank))
}

serp_results <- do.call(rbind, results_out)
serp_results$Cost <- if_else(serp_results$TP == 0, 
                             rgamma(nrow(serp_results), shape = 110.314, scale = 0.172) * (3.19 * 0.85), 
                             0)

# Results are for only a small period of the day; extrapolate to annual

```

### Approach 2: Try something different (rank vs threshold)

In addition to repeating the simulation results (as per above), we can also see what happens if we use a ranking approach instead of thresholds. The intention here is to check whether ranking by predicted risks leads to lower misclassification costs and better PPV based on the number of patients evaluated (x-axis). This can be compared to threshold costs.
...
for (i in 1:max_iter) {
  ...
  ranking_out[[i]] <- tibble(
    ...
    Mean_cost = c(
      (5 - sum(df[1:5,]$outcome_icu)) * FP +
        sum(df[6:n_samples,]$outcome_icu) * FN,
      (10 - sum(df[1:10,]$outcome_icu)) * FP +
        sum(df[11:n_samples,]$outcome_icu) * FN,
      (15 - sum(df[1:15,]$outcome_icu)) * FP +
        sum(df[16:n_samples,]$outcome_icu) * FN,
      (20 - sum(df[1:20,]$outcome_icu)) * FP +
        sum(df[21:n_samples,]$outcome_icu) * FN,
    ...
    Mean_PPV = c(
      mean(df[1:5,]$outcome_icu),
      mean(df[1:10,]$outcome_icu),
      mean(df[1:15,]$outcome_icu),
      mean(df[1:20,]$outcome_icu),
