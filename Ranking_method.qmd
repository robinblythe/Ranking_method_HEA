...
# NOTE - THE COST OF A FALSE POSITIVE IS PROBABLY TOO LOW HERE - NEED A GOOD OPPORTUNITY COST OR SOMETHING?

##### Fixed

```{r}
fx_nmb <- get_nmb_sampler(
  outcome_cost = 14134 + 3.19 * 19,
  wtp = wtp,
  qalys_lost = 0.03,
...
```{r}
cost_eval <- function() rgamma(1, shape = 110.314, scale = 0.172) * 3.19

fx_nmb_sampler <- get_nmb_sampler(
 outcome_cost = function() rnorm(1, 14134, 686) + cost_eval(),
 wtp = wtp,
 qalys_lost = function() rnorm(1, 0.03, 0.04),
 high_risk_group_treatment_cost = cost_eval(),
